...
**日期**：周五 · 第 54 期
**评分均值**：8.7
**主线**：自我加速第一次被量化，代价也第一次被记账 —— Anthropic 交出「研发自动化指数」：Claude 已主导自家 26% 的 AI 研发；Meta 把个人智能体 Muse 推上 Mac 直接操作本地文件；Figure 让机器人第一次走进 30 个陌生家庭就能干活（9% → 56%）；华为 NPO 超节点昇腾 960 提前三个季度；纽约时报案解封文件里微软高管把 AI 抓取写成「人类历史上最大规模的劳动窃取」
**H5 链接**：https://wangqi422.github.io/catwang-llm-wiki/docs/ai-daily/ai-daily-card-20260918-toc.html
**海报路径**：docs/ai-daily/ai-daily-poster-20260918.png（552.6KB / 1440×1920）
**URL 校验**：curl 轮询第 3 轮 200（前两轮 404 为 Pages 未就绪）+ 内容二次确认（title 0918、8 卡齐全）；海报 PNG 公网 URL 亦 200
**Commits**：`7b3ddb1`（MD/H5/归档）、`9c8aa14`（海报）
**信源**：AI HOT 5 页翻完共 457 条（hasNext=false），WebSearch 交叉验证 8 条

### 8 条日报
| # | 标题 | 评分 | 重要性 | 章节 |
|---|------|------|------|------|
| 1 | ⭐ Anthropic 发布前沿 AI 研发进度测量体系：Claude 已主导 26% 内部 AI 研发 | 9.2 | P0 | §1 模型&Agent |
| 2 | ⭐ Meta 发布 Muse for Mac，个人智能体直接接管本地电脑 | 9.0 | P0 | §1 |
| 3 | ⭐ Figure 发布 Helix 2.5：30 个陌生家庭零样本成功率 9% → 56% | 9.0 | P0 | §1 |
| 4 | ⭐ 华为发布业界首个 NPO 超节点昇腾 960，960DT 提前三个季度 | 8.9 | P0 | §2 行业&基础设施 |
| 5 | ⭐ 纽约时报案解封文件：微软高管称 AI 抓取是「史上最大劳动窃取」 | 8.8 | P0 | §2 |
| 6 | ⭐ OpenAI 被曝接近攻克霍奇猜想，或成第二道千禧年难题 | 8.7 | P0 | §2 |
| 7 | 阿里发布 Qwen3.8-Omni-Flash：1M 上下文全模态，音频输入成本降 98% | 8.2 | P1 | §3 趋势&工具 |
| 8 | SemiAnalysis：智能体流量已占全部推理流量 70% 以上 | 7.9 | P1 | §3 |

> 备注：Figure 56% 当日遭 Physical Intelligence 的 Tony Zhao 公开质疑；纽约时报案细节多来自原告方摘要（原件仍封存）；Qwen 数据为厂商自述且存在 64K 文档的口径冲突 —— 三处均在正文中标注。

---

## [2026-09-17 09:47] AIGC 日报 2026.09.17（五阶段全流程 · xiaoqi-ai-daily skill）

**推送结果**：✅ 2 群全成功（群 A + 群 B）
**日期**：周四 · 第 53 期
**评分均值**：8.5
**主线**：一边公开家丑，一边推高估值 —— OpenAI 首次系统性披露六起失对齐案例并明说「行业没解决对齐」；同日被曝 1.2 万亿美元估值融资（公司还想 1.5 万亿）+ Anthropic 三合一 Claude + Cohere×Aleph Alpha 200 亿合并
**H5 链接**：https://wangqi422.github.io/catwang-llm-wiki/docs/ai-daily/ai-daily-card-20260917-toc.html
**海报路径**：docs/ai-daily/ai-daily-poster-20260917.png（549.1KB / 2880×3840）
**URL 校验**：curl 首轮 200 + WebFetch 内容完整性二次确认（title 0917、8 卡齐全、三章节正确）；H5 与海报公网 URL 均 200
**Commits**：`7508c52`（MD/H5/归档）、`024c329`（海报）

### 8 条日报
| # | 标题 | 评分 | 重要性 | 章节 |
|---|------|------|------|------|
| 1 | ⭐ OpenAI 发布模型失对齐披露框架并公开六份报告：27 份被污染的摘要、2.15% 的隐瞒指令、「你自由了」 | 9.3 | P0 | §1 模型&Agent |
| 2 | Anthropic 合并 Cowork 与聊天为统一 Claude，推出 Claude Docs / Slides / Design | 8.8 | P0 | §1 |
| 3 | OpenAI 测试 Sponsored Agents：广告会「回话」了，HubSpot 与 Shopify 成为首批 CRM / 电商伙伴 | 8.4 | P0 | §1 |
| 4 | OpenAI 拟以 1.2 万亿美元估值 IPO 前再融资，公司自己还想再高到 1.5 万亿 | 8.9 | P0 | §2 行业&基建 |
| 5 | Cohere 与 Aleph Alpha 签署最终合并协议：约 200 亿美元、柏林多伦多双总部 | 8.3 | P0 | §2 |
| 6 | Mozilla 第二版《开源 AI 现状》91 页：开源落后 4 个月、中美差距 4.4 个月、20% 用量只换 4% 收入 | 7.9 | P1 | §2 |
| 7 | 腾讯开源 BrowserSkill：让 Agent 直接借用你已登录的真实浏览器标签页 | 8.2 | P1 | §3 趋势&工具 |
| 8 | 苹果详解 iPhone 18 Pro 参考图像：传感器固件签名生成不可篡改「数字底片」 | 7.9 | P1 | §3 |

### 本期亮点
- **情报**：AI HOT 5 页 438 条完整覆盖 24h（09-16T01:30 — 09-17T01:13 UTC），4 轮 WebSearch 交叉核实（失对齐框架 / 1.2 万亿融资 / Claude 合并 / Cohere 合并 / ChatGPT Ads / 苹果参考图像）
- **查重剔除**：阿里云 Wan3.0（实为 8 月 6 日公测旧闻，已在 08-25 期报道，AI HOT 09-16 条目为云栖大会二次传播）
- **坑位**：模板 head 沿用 1–142 行（`</head>` 在 142 行），title 已单独 sed 修正为 0917，本次未复现残留日期问题
- **海报**：2880×3840 / 549.1KB，分数回填正常（均分 8.5；02/03 为次要卡片按模板不显示分数，非 bug）


## [2026-09-16 09:52] AIGC 日报 2026.09.16（五阶段全流程 · xiaoqi-ai-daily skill）

**推送结果**：✅ 2 群全成功（群 A + 群 B）
**日期**：周三 · 第 52 期
**评分均值**：8.4
**主线**：嘴上要刹车，账上在狂飙 —— 三巨头安全协调被证实 + Anthropic 年化 650 亿 7 倍增长 + 英伟达把 Fable 挡在核心数据外
**H5 链接**：https://wangqi422.github.io/catwang-llm-wiki/docs/ai-daily/ai-daily-card-20260916-toc.html
**海报路径**：docs/ai-daily/ai-daily-poster-20260916.png（660.2KB / 3:4）
**URL 校验**：curl 首轮 200（40958 字节与本地一致）+ WebFetch 内容完整性二次确认（8 卡齐全、返回归档按钮在位）

### 8 条日报
| # | 标题 | 评分 | 重要性 | 章节 |
|---|------|------|------|------|
| 1 | ⭐ Google 发布 Gemini 3.8 Live 与 3.8 Live Extended Thinking：边说边推理边调工具，语音对语音榜 82.6 登顶 | 9.0 | P0 | §1 模型&Agent |
| 2 | ⭐ Meta One 七档订阅全球上线：$2.99–$499/月，把 Muse 生成额度打包进 30 亿用户的社交图谱 | 8.2 | P0 | §1 |
| 3 | 生数科技发布 Vidu S2 双模型：实时互动 720P + 实时编辑视频流 + 空间视频，发布当天全量开放 | 8.0 | P1 | §1 |
| 4 | ⭐ Anthropic 年化收入已达 650 亿美元、同比增长约 7 倍，2 万亿 IPO 与英伟达 100 亿锚定投资照常推进 | 8.9 | P0 | §2 行业&基建 |
| 5 | ⭐ 英伟达、Palantir、博思艾伦限制使用 Anthropic Fable：敏感业务改跑自家 Nemotron，Palantir 索要「不可撤销」零数据保留 | 8.5 | P0 | §2 |
| 6 | 谷歌 TPU 互联迈入 100 万芯片级，CTO 瓦赫达特：AI 算力瓶颈已从「缺芯」转向「缺电」 | 8.1 | P1 | §2 |
| 7 | ⭐ OpenAI 证实与 Anthropic、Google DeepMind 就 AI 安全协调数周，FTC 主席批「筑护城河」 | 8.4 | P0 | §3 趋势&工具 |
| 8 | Strix 自主攻防智能体 25 分钟拿到 Baseten 生产 GitHub 管理员令牌：一枚 2023 年的构建参数泄漏了三年多 | 7.8 | P1 | §3 |

### 关键产出
- MD：`raw/daily-reports/AIGC_Daily_Report_20260916.md`
- H5：`docs/ai-daily/ai-daily-card-20260916-toc.html`（41.0KB）
- 归档：DATA 96 条，0916 在首位
- git：`46f7246`（MD/H5/归档）→ `8406d62`（title 修正）→ `7371b0a`（海报）
- 情报：AI HOT 4 页 400 条 + 6 轮定向核实

### 坑位
- **模板 head 必须取 1–142 行**：0915 模板 `</head>` 在第 142 行，取少会丢闭合标签（本次已按 142 行切片并 grep 复核通过）
- **H5 `<title>` 会残留模板日期**：复用模板 head 时 `<title>` 是硬编码的 `2026.09.15`，必须单独改。本次 WebFetch 内容校验时才发现（标题显示成 0915），已修并补推 `8406d62`
- **AI HOT p4 需用 p3 的 nextCursor**：p3 解析时若忘了写回 cursor 文件，p4 会重复返回 p3 的 100 条。本次已修正，4 页覆盖 09-15T09:45 → 09-16T01:18 完整 24h
...
**评分均值**：8.3
**主线**：前方踩刹车，后方在漏油
**H5 链接**：https://wangqi422.github.io/catwang-llm-wiki/docs/ai-daily/ai-daily-card-20260913-toc.html
**海报路径**：docs/ai-daily/ai-daily-poster-20260913.png（552.1KB / 3:4）

### 8 条日报
| # | 标题 | 评分 | 重要性 | 章节 |
|---|------|------|------|------|
| 1 | ⭐ Dario Amodei《We Must Pace the Frontier》：承认 RSI 已出现，提出三步刹车法 | 9.2 | P0 | §1 模型&Agent |
| 2 | Cognition 发布 SWE-2：Kimi K3 后训练，成本降 64% 逼近 Fable 5.1 | 8.0 | P1 | §1 |
| 3 | Meta《Jagged Judges》：对抗性 LLM 可说服判官翻转 62–91% 判决，70% 翻离真相 | 8.2 | P1 | §1 |
| 4 | ⭐ 三巨头罕见同框：Altman 同意放缓并承诺引入独立评估者，确认 2026 不 IPO | 9.0 | P0 | §2 行业&基建 |
| 5 | ⭐ 6TB 中转站日志流入黑市：SSH 私钥与云凭证可接管 19 家企业 + 7 家政企机构 | 8.8 | P0 | §2 |
| 6 | 70+ 英国议员联名：支持全球首个禁止超级智能立法，呼吁 G20 组「志愿联盟」 | 7.9 | P1 | §2 |
| 7 | OpenAI Habitat 重写 Rust：2 名工程师 + Codex + GPT-5.5，CPU 效率提升 6 倍 | 7.9 | P1 | §3 趋势&工具 |
| 8 | 谷歌 15 亿美元人才收购 Mechanize：规避反垄断审查，补齐 AI 编程短板 | 7.5 | P1 | §3 |

### 执行记录
- **阶段 1 情报采集**：AI HOT 聚合池 3 页分页共 **269 条**（`since=2026-09-12T00:00:00Z`，cursor 翻至 hasNext=false）；8 轮定向 WebSearch 事实核查
- **阶段 2 MD**：`raw/daily-reports/AIGC_Daily_Report_20260913.md`（244 行）
- **阶段 3 H5 + 归档**：复用 20260912 模板 head 142 行；归档首页 DATA 顶部追加；git push `811135e`
- **阶段 4 URL 校验**：✅ HTTP 200（第 2 次检测通过）
- **阶段 5 海报 + 推群**：`publish-ai-daily-poster.js --date 20260913` 一条龙，git push `0f0ab56`，Pages 第 4 次检测就绪，markdown_v2 单条 209 字节，2/2 群成功

### 本次要点
- **查重规避**：「英伟达/Anthropic IPO」昨日 #5 已报、「RubyGems」「菲尔兹奖」均已覆盖，本期全部避开并替换为英国禁 ASI 立法
- **事实核查**：6TB 泄露事件媒体定位为「研究者单方声明」，MD 中已明确标注保留说明（无企业官方通报）
- **合规处理**：涉及小米/华为等企业仅客观呈现，标注为风险警示而非已确认入侵
...
**日期**：周二 · 第 44 期
**评分均值**：7.8
**H5 链接**：https://wangqi422.github.io/catwang-llm-wiki/docs/ai-daily/ai-daily-card-20260908-toc.html
**海报路径**：docs/ai-daily/ai-daily-poster-20260908.png（511.7KB / 1440×1920）

### 8 条日报
| # | 标题 | 评分 | 重要性 |
|---|------|------|------|
| 1 | GPT-6 Astra 自主通关《传送门》：3336 次工具调用、$571、24 小时 | 8.6 | P0 |
| 2 | 华为时隔六年发布麒麟 9050 Pro：逻辑折叠「韬定律」+ 鸿蒙 7 | 8.5 | P0 |
| 3 | 《西雅图时报》+《新闻日报》起诉 OpenAI/微软，要求销毁模型 | 7.7 | P0 |
| 4 | 微软论文：LLM Agent 16 步长程任务崩到 0–33%，成功服从几何衰减 | 7.6 | P1 |
| 5 | 面壁智能开源 MiniCPM5-2B：4B 以下开源榜首，Apache 2.0 + vLLM Day-0 | 7.5 | P1 |
| 6 | AMD + vLLM：MI355X 智能体负载 19 天性能暴涨 11 倍 | 7.4 | P1 |
| 7 | NVIDIA Sol-H3：MiniMax-H3 视频生成速度首次超过播放速度 | 7.5 | P1 |
| 8 | OpenAI ChatGPT Work 推出个人写作风格学习功能 | 7.3 | P1 |
